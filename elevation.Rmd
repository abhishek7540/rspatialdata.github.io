...
- **USGS Elevation Point Query Service** for point elevation data.

- **Amazon Web Services Terrain Tiles** is for the raster elevation data such as DEM (digital elevation model).
...
ele <- as.data.frame(elevation_data,xy=TRUE)

colnames(ele)[3] = "elevation" 

ggplot() +
  geom_raster(data = ele , aes(x = x, y = y, fill = elevation)) + 
  geom_sf(data = swiss_bound, color = "black", fill = NA)+
