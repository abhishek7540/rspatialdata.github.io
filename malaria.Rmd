...
This is a tutorial for the `mapAtlas` package which allows users to download, visualise and manipulate global parasite rate survey data(Plasmodium and P.vivax) and modeled raster outputs. This package is to provide users with a convenient API to interact with the publicly available parasite data.


## Installation of `malariaAtlas` package

To download the data and the other functions required to visualise the same, you need the `malarisAtlas` package, which can be downloaded from CRAN using
...
The data can be explored interactively at [data explorer](https://malariaatlas.org/explorer/#/). This can also be used for finding more information on raster data. It consists of a dashboard which the shows the spread of the parasite over time and also the option for downloading the data used.

## List of datasets present in the package

The `listData()` function from the `mapAtlas` package helps retrieve a list of all the available data which can be downloaded.
...
3. `autoplot.pr.points` is an autoplot method to enable quick visualising of the locations of downloaded PR points. It can be used thus:
...
3. `autoplot.vector.points`: is an autoplot method to quickly and easily visualise the vector occurrence data for a specific country.It can be used thus:
...
- **Modelled Rasters**
 ...
 `getRaster()` : downloads publicly available MAP rasters for a specific surface and year, clipped to a given bounding box or shapefile. For example for Zimbabwe, we use the `getRaster` function as:
...
```{r message = FALSE}
ZMB_shp <- getShp(country = "Zimbabwe", admin_level = "admin0") #get shapefile data
ZMB_shp_df <- as.MAPshp(ZMB_shp) #convert the shapefile data
ZMB_PfPR2_10 <- getRaster(surface = "Plasmodium falciparum PR2-10", shp = ZMB_shp, year = 2013) #get the raster data
ZMB_PfPR2_10_df <- as.MAPraster(ZMB_PfPR2_10) #convert the raster data

p <- autoplot(ZMB_PfPR2_10_df, shp_df = ZMB_shp_df, printed = FALSE) #plot the raster data for the country using shapefile data

pr <- getPR(country = c("Zimbabwe"), species = "Pf") #get the PR data for the country
p[[1]] +
...
```{r message = FALSE}
IND_shp <- getShp(country = "India", admin_level = "admin0") #get shapefile data
IND_shp_df <- as.MAPshp(IND_shp) #convert shapefile data 
IND_an_dirus <- getRaster(surface = "Anopheles dirus species complex", shp = IND_shp) #get raster data
IND_an_dirus_df <- as.MAPraster(IND_an_dirus) #convert raster data

p <- autoplot(IND_an_dirus_df, shp_df = IND_shp_df, printed = FALSE) # plot the raster data on maps using shapefile

vec <- getVecOcc(country = c("India"), species = "Anopheles dirus") #get the vector point data for country
p[[1]] +
