...
[`wopr`](https://github.com/wpgp/wopr) is an R package that provides access to the [WorldPop Open Population Repository](https://wopr.worldpop.org/) and gets estimates of population sizes for specific geographic areas
...
To download the population data we first retrieve the WOPR data catalogue and then subset the data frame for the country we are interested in.

To see a list of currently available databases in the catlogue we can use the `getCatalogue()` function.
...
WOPR uses [ISO country codes](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-3) to abbreviate country names. In this tutorial we use 'NGA' which refers to Nigeria.

Then we simply subset the catalogue for Nigeria(NGA) and download the data for the subsetted catalogue selection using the `downloadData()` function. Note that `downloadData()` will not download files larger than 100MB by default unless the `maxsize` argument is changed (see `?downloadData`)
...
We will also use the `here` package to get file paths to the working directory, so that we can simply use relative paths to import files placed within the working directory.
...
Then the `ggplot2` package can be used to plot the administrative boundaries and visualise the population. The `geom_sf()` function in the `ggplot2` package allows us to easily visualise simple feature objects. But for this, we will have to convert the shapefile we have into a simple feature object. We can do this by using the `st_as_sf()` function in the `sf` package.

We also color the administrative divisions in proportion to its population as follows. Note how we have used the variable 'mean' to fill in the divisions, and how the legend on the right of the map describes the color scale used.
...
The following example uses the downloaded the population of Nigeria and the `sf` and `ggplot2` packages to visualise the population using a choropleth map. Note how each administrative division in Nigeria has been colored in proportion to its population.

```{r choropleth-adm-lvl2-example}
#install.packages(c("rgdal", "here", "sf", "ggplot2"))
library(rgdal)
library(here)
library(sf)
library(ggplot2)
...
#install.packages(c("rgdal", "here", "leaflet"))
library(rgdal)
library(here)
library(leaflet)
...
A downside of choropleth maps is that geographical areas which are bigger in size tend to have a bigger weight on the map, and creates a bias towards that geographical area. To prevent this bias, we use cartograms.
...
#install.packages(c("rgdal", "here", "cartogram", "sf", "ggplot2"))
library(rgdal)
library(here)
library(cartogram)
library(sf)
...
# converting the admin_level2_shape object to an object class sf
admin_level2_sf <- st_as_sf(admin_level2_shape)
