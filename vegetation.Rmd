...
```{r eval=FALSE}
library(MODIStsp)
spatial_file <- here::here("mongolia_administrative_boundaries_national_polygon/mongolia_administrative_boundaries_national_polygon.shp")
...
NDVI_raster <- raster(here::here("VegetationData/mongolia_administrative_boundaries_national_polygon/VI_16Days_1Km_v6/NDVI/MOD13A2_NDVI_2020_161.tif"))
# Converting the raster object into a dataframe
NDVI_df <- as.data.frame(NDVI_raster, xy = TRUE, na.rm = TRUE)
rownames(NDVI_df) <- c()

ggplot() +
  geom_raster(data = NDVI_df, aes(x=x,y=y,fill = MOD13A2_NDVI_2020_161))
